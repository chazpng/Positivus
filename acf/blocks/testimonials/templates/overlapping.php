	...
	<div class="bg-gray-900 pb-20 sm:pb-24 xl:pb-0">
		<div class="mx-auto flex max-w-7xl flex-col items-center gap-x-8 gap-y-10 px-6 sm:gap-y-8 lg:px-8 xl:flex-row xl:items-stretch">
			...
			<div class="relative aspect-[2/1] h-full md:-mx-8 xl:mx-0 xl:aspect-auto">
				<?php if ( ! empty( $avatar ) ) : ?>
					<img class="absolute inset-0 h-full w-full rounded-2xl bg-gray-800 object-cover shadow-2xl" src="<?php echo esc_url( $avatar['url'] ); ?>" alt="<?php echo esc_attr( $avatar['alt'] ? $avatar['alt'] : 'Avatar' ); ?>">
				<?php endif; ?>
			</div>
			</div>
			<div class="w-full max-w-2xl xl:max-w-none xl:flex-auto xl:px-16 xl:py-24">
				<figure class="relative isolate pt-6 sm:pt-12">
					...
					<?php if ( ! empty( $quotee ) ) : ?>
						<div class="font-semibold text-white"><?php echo esc_html( $quotee ); ?></div>
					<?php endif; ?>
					<?php if ( ! empty( $position ) ) : ?>
						<div class="mt-1 text-gray-400"><?php echo esc_html( $position ); ?></div>
					<?php endif; ?>
					</figcaption>
				</figure>
			</div>
