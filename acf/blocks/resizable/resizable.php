...
$uniqid         = uniqid();
$disable_resize = get_field( 'disable_resize' ) ?: 'resize-x';
$hide_code      = get_field( 'hide_code' ) ? 'hidden' : '';
?>
<div class="gl-b-resizable container mx-auto" id="<?php echo esc_attr( $uniqid ); ?>">
	<div class="m-auto max-w-screen-2xl items-center justify-center">
		<div class="flex justify-between">
			...
			<div class="flex items-center <?php echo esc_attr( $hide_code ); ?>">
				<div class="flex gap-x-2.5 rounded-lg bg-gray-50 p-1 ring-1 ring-gray-100">
					<button class="toggle-button preview-button active group relative hidden items-center justify-center gap-x-2 rounded-lg bg-white px-3 py-2 text-sm font-semibold text-gray-700 shadow-sm sm:flex" data-id="<?php echo esc_attr( $uniqid ); ?>">
						Preview
					</button>
					<button class="toggle-button code-button group relative hidden items-center justify-center gap-x-2 rounded-lg bg-transparent px-3 py-2 text-sm font-semibold text-gray-500 sm:flex" data-id="<?php echo esc_attr( $uniqid ); ?>">
						Code
					</button>
				</div>

				<button class="copy-code group relative ml-2 hidden items-center justify-center gap-x-2 rounded-lg px-4 py-3 text-sm font-medium text-gray-700 shadow-sm ring-1 ring-gray-200 hover:bg-slate-200/20 sm:flex" data-id="<?php echo esc_attr( $uniqid ); ?>">
					<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
